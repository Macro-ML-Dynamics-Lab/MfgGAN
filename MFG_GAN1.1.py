...
datarho = pd.read_csv('//home/naman//MFG/Density_Data.csv').values
datau = pd.read_csv('//home//naman//MFG//Velocity_Data.csv').values
dataomega = pd.read_csv('//home//naman//MFG//Omega_Data.csv').values
dataV = pd.read_csv('//home//naman/MFG//Cost_data.csv').values
datautilde = pd.read_csv('//home/naman//MFG//wvelocity_Data.csv').values
...
class MFG_PIGAN:
	def __init__(self, X_CE, rho, u, omega, X_HJB, V, utilde, aux, lbce, ubce, lbhjb, ubhjb, layersg, layersu, layersd):
		...
		self.gen_loss = tf.reduce_mean(tf.square(self.rhop-self.rho_pred))+tf.reduce_mean(tf.square(self.omegap-self.omega_pred))+tf.reduce_mean(tf.square(self.ce_res1))+tf.reduce_mean(tf.square(self.ce_res2))
		self.dis_loss = tf.reduce_mean(tf.square(self.Vp-self.V_pred))+tf.reduce_mean(tf.square(self.utildep-self.utilde_pred))+tf.reduce_mean(tf.square(self.hjb_res))
	...
	def HJB(self, x, t, w):
	 	...
	 	U_eq = u_max*(omega/u_max-rho/rho_max)
	 	f = dV_t+U_eq*dV_x-1/2*(dV_x-lamda*dV_w)**2
...
pd.DataFrame(U_pred).to_csv('//home//naman//MFG//U_Predict_coarse.csv', index = False)
pd.DataFrame(omega_pred).to_csv('//home//naman//MFG//omega_Predict_coarse.csv', index = False)
pd.DataFrame(rho_pred).to_csv('//home//naman//MFG//rho_Predict_coarse.csv', index = False)

x = np.linspace(xlo, xhi, 241)
t = np.linspace(tlo, 3.5, 961)
# w = np.linspace(wlo, whi, 241)
...
rho_pred = Y_pred[:, 0:1]
omega_pred = Y_pred[:, 1:2]
# utilde_pred = f_pred[1]# U_pred = model.get_velocity_field(data)
pd.DataFrame(U_pred).to_csv('//home//naman//MFG//U_Predict.csv', index = False)
pd.DataFrame(omega_pred).to_csv('//home//naman//MFG//omega_Predict.csv', index = False)
pd.DataFrame(rho_pred).to_csv('//home//naman//MFG//rho_Predict.csv', index = False)