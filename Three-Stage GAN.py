...
sigma = 0.01
datarho = pd.read_csv('/home/naman/GAN/Datasets/stochrho'+str(sigma)+'.csv').values
datau = pd.read_csv('/home/naman/GAN/Datasets/stochu'+str(sigma)+'.csv').values
dataomega = pd.read_csv('/home/naman/GAN/Datasets/stochxi'+str(sigma)+'.csv').values
dataV = pd.read_csv('/home/naman/GAN/Datasets/stochC'+str(sigma)+'.csv').values
datautilde = pd.read_csv('/home/naman/GAN/Datasets/stochutilde'+str(sigma)+'.csv').values

datautilde = datautilde.reshape(320, 80, 80)
dataV = dataV.reshape(320, 80, 80)
...
Nce = uxn+utn*2
Nhjb = uwn*uxn+2*utn*uxn+2*utn*uwn
Nf = np.random.randint(10000, 25000)
...
rho0 = datarho[0:1, :].reshape((uxn, 1))
rho1 = datarho[:, 0:1].reshape((utn, 1))
rho2 = datarho[:, -1:].reshape((utn, 1))
omega0 = dataomega[0:1, :].reshape((uxn, 1))
omega1 = dataomega[:, 0:1].reshape((utn, 1))
omega2 = dataomega[:, -1:].reshape((utn, 1))
u0 = datau[0:1, :].reshape((uxn, 1))
u1 = datau[:, 0:1].reshape((utn, 1))
u2 = datau[:, -1:].reshape((utn, 1))

xhjb0 = np.hstack([XHJB[-1:, :, :].reshape((uxn*uwn,1)), THJB[-1:, :, :].reshape((uxn*uwn,1)), WHJB[-1:, :, :].reshape((uxn*uwn,1))])
xhjb1 = np.hstack([XHJB[:, 0:1, :].reshape((utn*uwn, 1)), THJB[:, 0:1, :].reshape((utn*uwn, 1)),  WHJB[:, 0:1, :].reshape((utn*uwn, 1))])
xhjb2 = np.hstack([XHJB[:, -1:, :].reshape((utn*uwn, 1)), THJB[:, -1:, :].reshape((utn*uwn, 1)),  WHJB[:, -1:, :].reshape((utn*uwn, 1))])
xhjb3 = np.hstack([XHJB[:, :, 0:1].reshape((utn*uxn, 1)), THJB[:, :, 0:1].reshape((utn*uxn, 1)),  WHJB[:, :, 0:1].reshape((utn*uxn, 1))])
xhjb4 = np.hstack([XHJB[:, :, -1:].reshape((utn*uxn, 1)), THJB[:, :, -1:].reshape((utn*uxn, 1)),  WHJB[:, :, -1:].reshape((utn*uxn, 1))])

V0 = dataV[-1:, :, :].reshape((uxn*uwn, 1))
V1 = dataV[:, 0:1, :].reshape((utn*uwn, 1))
V2 = dataV[:, -1:, :].reshape((utn*uwn, 1))
V3 = dataV[:, :, 0:1].reshape((utn*uxn, 1))
V4 = dataV[:, :, -1:].reshape((utn*uxn, 1))
utilde0 = datautilde[-1:, :, :].reshape((uxn*uwn, 1))
utilde1 = datautilde[:, 0:1, :].reshape((utn*uwn, 1))
utilde2 = datautilde[:, -1:, :].reshape((utn*uwn, 1))
utilde3 = datautilde[:, :, 0:1].reshape((utn*uxn, 1))
utilde4 = datautilde[:, :, -1:].reshape((utn*uxn, 1))

X_CE_train1 = np.vstack([xce0, xce1, xce2])
rho_train1 = np.vstack([rho0, rho1, rho2])
pd.DataFrame(rho_train1).to_csv('/home/naman/GAN/Results/trainrho.csv')
omega_train1 = np.vstack([omega0, omega1, omega2])
u_train1 = np.vstack([u0, u1, u2])
...
class MFG_PIGAN:
	def __init__(self, X_CE, rho, u, omega, X_HJB, V, utilde, aux, lbce, ubce, lbhjb, ubhjb, layersg, layersu, layersd):
		...
		self.gen_loss = 10*tf.reduce_mean(tf.square(self.rhop-self.rho_pred))+10*tf.reduce_mean(tf.square(self.omegap-self.omega_pred))+tf.reduce_mean(tf.square(self.ce_res1))+tf.reduce_mean(tf.square(self.ce_res2))
		...
		self.u_loss = tf.reduce_mean(tf.square(self.up-self.u_pred))+tf.reduce_mean(tf.square(self.u_res_pred-self.u_res))
		...
		self.d_optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.dis_loss, method = 'L-BFGS-B', options = {'maxiter': 300,'maxfun': 50000,'maxcor': 50,'maxls': 50,'ftol' :1.0 * np.finfo(float).eps})
		self.g_optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.gen_loss, method = 'L-BFGS-B', options = {'maxiter': 300,'maxfun': 50000,'maxcor': 50,'maxls': 50,'ftol' :1.0 * np.finfo(float).eps})
		self.u_optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.u_loss, method = 'L-BFGS-B', options = {'maxiter': 300,'maxfun': 50000,'maxcor': 50,'maxls': 50,'ftol' :1.0 * np.finfo(float).eps})
	...
	def CE(self, x, t):
	 	...
	 	f1 = drho_t+drhou_x-0.001*drho_xx
	...
	# 			if (epoch+1)%100 == 0:
	# 				lr = lr/10
	# 			sess.run(self.dis_optimizer, feed_dict={self.lr: lr, self.xhjbp: self.xhjb, self.thjbp: self.thjb, self.whjbp: self.whjb, self.Vp: self.V, self.utildep: self.utilde,
	...
	def callback_dis(self, loss):
		self._last_dis_loss = loss
		print('Loss:', loss)

	def callback_gen(self, loss):
		self._last_gen_loss = loss
		print('Loss:', loss)

	def callback_u(self, loss):
		self._last_u_loss = loss
		print('Loss:', loss)
	...
	def train_GAN_lbfgsb(self, epochs):
		...
		# FIX 2: removed stray `feed_dict=` and added missing Awp
		feed_dict2 = {self.xcep: self.xce, self.tcep: self.tce, self.rhop: self.rho, self.omegap: self.omega,
					self.up: self.u, self.Axp: self.Ax, self.Atp: self.At, self.Awp: self.Aw}
		feed_dict3 = {self.xcep: self.xce, self.tcep: self.tce, self.up: self.u, self.Axp: self.Ax, self.Atp: self.At, self.Awp: self.Aw}
		for epoch in range(epochs):
			self._last_dis_loss = None
			self._last_gen_loss = None
			self._last_u_loss   = None
			print('Training the HJB Equation:...')
			self.d_optimizer.minimize(self.sess, feed_dict = feed_dict1, fetches = [self.dis_loss], loss_callback = self.callback_dis)
			print('Training the Continuity Equation:...')
			self.g_optimizer.minimize(self.sess, feed_dict = feed_dict2, fetches = [self.gen_loss], loss_callback = self.callback_gen)
			print('Training the Mean Field Velocity:...')
			self.u_optimizer.minimize(self.sess, feed_dict = feed_dict3, fetches = [self.u_loss], loss_callback = self.callback_u)
			self.dis_loss_history.append(self._last_dis_loss)
			self.gen_loss_history.append(self._last_gen_loss)
			self.u_loss_history.append(self._last_u_loss)
		loss_df = pd.DataFrame({'epoch': range(1, epochs + 1), 'hjb_loss': self.dis_loss_history, 'ce_loss': self.gen_loss_history, 'u_loss': self.u_loss_history})
		loss_df.to_csv('/home/naman/GAN/Losses/loss_history'+str(se)+'.csv', index=False)
...
layersg = [2, 32, 32, 32, 32, 32, 32, 2]
layersd = [3, 32, 32, 32, 32, 32, 32, 1]
layersu = [2, 32, 32, 32, 32, 32, 32, 1]
...
model.train_GAN_lbfgsb(epochs = 50)
elapsed = time.time() - initial_time
print('Training time: %.4f seconds' % elapsed)
Y_pred, V_pred, U_pred = model.predict(inputCE, inputHJB)
rho_pred = Y_pred[:, 0:1]
omega_pred = Y_pred[:, 1:2]
pd.DataFrame(U_pred).to_csv('/home/naman/GAN/Results/U_Predict'+str(se)+'.csv', index=False)
pd.DataFrame(omega_pred).to_csv('/home/naman/GAN/Results/omega_Predict'+str(se)+'.csv', index=False)
pd.DataFrame(rho_pred).to_csv('/home/naman/GAN/Results/rho_Predict'+str(se)+'.csv', index=False)